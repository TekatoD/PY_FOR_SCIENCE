...
def create_database(folder='folder', input_name='Image', ouput_name='Depth', samples_num=250, size=(160, 120)):
    ...
    x = [np.empty((samples_num - arrays_num + 1, *size), dtype=np.float32) for arr in range(arrays_num)]
    y = np.empty((samples_num - arrays_num + 1, *size), dtype=np.float32)
    for iter in range(samples_num - arrays_num + 1):
        for arr in range(arrays_num):
            x[arr][iter] = png_scaler_grayer(frmt.format(path=folder, file=input_name, val=(iter + arr), ext='exr'), *size)
        y[iter] = exr_scaler_grayer(frmt.format(path=folder, file=ouput_name, val=iter + arrays_num - 1, ext='png'), *size)
    return x, y
